<?
/**
 * PHP Calendar Class w/Google v1.2.3
 *
...
*/
	...
	*/
		...
		var $jqversion = '1.9.0';                 // version of jquery to load
		var $jquiversion = '1.10.0';              // version of jquery ui to load
	...
	//--------------------------------------------------------------------------------------------
		function CALENDAR($type='full',$date='',$staticdisplaytype=false) { // types: mini,full,list,mini-list (default=full)
			...
			if($day == '') {
				$day = date('j'); // set to the 1st if year and month is known
			}
		...
		function set_js() {
			...
			$html = '
			<script type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/'.$this->jqversion.'/jquery.min.js?ver='.$this->jqversion.'"></script>
			<script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/jqueryui/'.$this->jquiversion.'/jquery-ui.min.js?ver='.$this->jquiversion.'"></script>
			';
		...
		function calendar_cell($day, $class, $date = '', $style = '') {
			...
			if($day != '') {
				...
				// Edited.
				// Get our ACF fields and insert them into events array.
				// Set up our djgig Loop using WP_Query
		...
		function addEvent($arr) {
			...
			$args = array (
					...
					// 'meta_key' 				 => 'djgigs_event_start_date', // order djgigs by date
					//'order' 				 => 'ASC',
					'posts_per_page'         => '-1', // get all djgig posts
				);
			...
			$sdate = get_field('djgigs_event_start_date'); 
			$startdate = strtotime(date("Y-m-d", strtotime($from)) . " +".$i." day");
			$startdate = date("Y-n-j",$date);
			$edate = get_field('djgigs_event_end_date');
			$enddate = get_field('djgigs_event_end_date');
